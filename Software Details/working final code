...
SHAKE_LABELS = ["Shake 1!", "Shake 2!", "Shake 3!"]
...
def show_result_screen(oled, pokemon_key, pokemon_name, caught):
    try:
        ...
        if frames:
            ...
            if caught:
                oled_message(oled, pokemon_name, "Caught!!", ":)")
            else:
                oled_message(oled, pokemon_name, "Got away!", ":(")
    except:
        if caught:
            oled_message(oled, pokemon_name, "Caught!!", ":)")
        else:
...
def try_catch(oled, strip, scan_btn, motor_ena, motor_in1, motor_in2, pokemon_key, pokemon_name):
    ...
    hold_shake_times = [1000, 2500, 4000]
...
def identify_and_play(adc_val, oled, strip):
    if   50  <= adc_val <= 80:    return play_snivy(oled, strip)
    elif 160 <= adc_val <= 190:   return play_vaporeon(oled, strip)
    elif 210 <= adc_val <= 245:   return play_raichu(oled, strip)
    elif 370 <= adc_val <= 420:   return play_umbreon(oled, strip)
    elif 510 <= adc_val <= 550:   return play_pikachu(oled, strip)
    elif 640 <= adc_val <= 690:   return play_mudkip(oled, strip)
    elif 750 <= adc_val <= 800:   return play_mew(oled, strip)
    elif 870 <= adc_val <= 910:   return play_jigglypuff(oled, strip)
    elif 980 <= adc_val <= 1010:  return play_gengar(oled, strip)
    elif 1080 <= adc_val <= 1110: return play_spheal(oled, strip)
    else:
...
def main():
    ...
    while True:
        ...
        while True:
            if check_long_press(boot_btn, LONG_PRESS_MS):
        ...
        strip_clear(strip)
        pokemon_key, pokemon_name = identify_and_play(adc_val, oled, strip)

        if pokemon_key is not None:
            strip_clear(strip)
            sleep_ms(300)
            try_catch(oled, strip, scan_btn,
