...
MONO_BUF_SIZE  = 512
LONG_PRESS_MS  = 2000
CATCH_PRESS_MS = 5000   # 5-second hold to throw pokeball
ADC_SAMPLES    = 20

# ─── Rarity Table ──────────────────────────────────────────────────────────────
# catch_chance: numerator out of 20 (so 15/20=common, 10/20=uncommon, 2/20=rare, 1/20=ultra)
RARITY = {
...
def check_long_press(btn, duration_ms):
    """Generic long press check for any button and any duration."""
    if btn.value() == 0:
        t = ticks_ms()
        while btn.value() == 0:
...
def try_catch(oled, strip, scan_btn, pokemon_key, pokemon_name):
    ...
    Full catch sequence:
    1. Show rarity
    2. Wait for 5s SCAN hold
    3. Roll random catch
    4. Show result + LED colour
    5. Return True if caught, False if not
    """
    ...
    # ── Countdown bar on LEDs while waiting for hold ───────────────────────────
    # Light LEDs one-by-one as hold progress (16 LEDs over 5000ms = ~312ms each)
    oled_message(oled, "Hold SCAN", "to throw!", "5 seconds...")
    strip_clear(strip)
    ...
    while True:
        if scan_btn.value() == 0:
            ...
            while led_lit < new_led:
                strip[led_lit] = (150, 80, 0)   # amber progress
                strip.write()
                led_lit += 1
            ...
            # Button released early — reset
            strip_clear(strip)
            led_lit = 0
            t_start = ticks_ms()
            oled_message(oled, "Hold SCAN", "to throw!", "Keep holding!")

        sleep_ms(30)

    wait_for_button_release(scan_btn)

    # ── Throw animation on OLED ───────────────────────────────────────────────
    strip_clear(strip)
    oled_message(oled, "Pokeball", "thrown!", "")
    sleep_ms(600)

    # Pokeball shake — flash white 3 times
    for _ in range(3):
    ...
    # ── Random catch roll ─────────────────────────────────────────────────────
    roll = urandom.getrandbits(8) % 20   # 0–19
    ...
    if caught:
        ...
        # Show caught screen with binary file sprite
        try:
            ...
            if frames:
                ...
                name_short = pokemon_name[:7]
                oled.text(name_short, 66, 10, 1)
        ...
        # Show escaped screen with binary file sprite
        try:
            frames = load_frames("/sd/" + pokemon_key + ".bin")
            if frames:
                fb = framebuf.FrameBuffer(frames[0], FRAME_WIDTH, FRAME_HEIGHT, framebuf.MONO_HLSB)
...
def identify_and_play(adc_val, oled, strip):
    """Returns (pokemon_key, pokemon_name) or (None, None) if unrecognised."""
    ...
    elif 350 <= adc_val <= 420:   return play_umbreon(oled, strip)
...
def main():
    ...
    while True:
        ...
        # Play Pokedex entry — returns (key, name)
        strip_clear(strip)
        ...
        # Auto-reboot to scan again
        strip_clear(strip)
        oled_message(oled, "Restarting...", "", "")
        sleep(2)
        gc.collect()
        # Loop back — no hard reset needed, just continues
