...
FRAME_FILE      = "/sd/gengar.bin"
WAV_FILE        = "/sd/gengar.wav"

DARK_PURPLE     = (250, 0, 250)
LIGHT_PURPLE    = (100, 0, 250)
...
def show_gengar_colors(strip):
    for i in range(4):
        strip[i] = DARK_PURPLE
    for i in range(4, 8):
        strip[i] = LIGHT_PURPLE
    strip.write()
...
def load_frames(path):
    ...
    with open(path, "rb") as f:
        while True:
            ...
            count += 1
            if count > 30:  # cap frames to avoid SD/memory stress
                break
    return frames
...
try:
    ...
    np = init_neopixel()
    show_gengar_colors(np)      # set once, never changed again
    ...
    if n_frames == 0:
        raise Exception("No frames found in gengar.bin")

    # Extra delay to stabilize SD card after mounting (fixes 'timeout waiting for response')
    sleep(1)
    ...
    print("Playing Gengar with constant NeoPixel...")
    ...
    while True:
        ...
        if loop_count % 6 == 0:
            if n_frames > 0:
                draw_frame(oled, frames[frame_i % n_frames])
                frame_i += 1
