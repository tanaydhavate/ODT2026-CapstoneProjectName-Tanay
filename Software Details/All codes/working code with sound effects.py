...
# ─── Rarity Table ──────────────────────────────────────────────────────────────
# catch_chance: numerator out of 20
RARITY = {
...
def play_wav_simple(path):
    """Play a WAV file with no animation — for caught/notcaught sounds."""
    audio = None
    wav   = None
    try:
        audio    = init_audio()
...
# ── Helper: draw the hold-progress screen with live countdown ─────────────────
...
def try_catch(oled, strip, scan_btn, pokemon_key, pokemon_name):
    ...
    # ── 5-second hold with amber LED progress bar + OLED countdown ────────────
    strip_clear(strip)

    t_start      = ticks_ms()
    led_lit      = 0
    last_secs    = -1       # track last displayed second to avoid flicker
    audio_started = False   # whether we've kicked off the result WAV
    audio        = None
    wav          = None
    mono_buf     = bytearray(MONO_BUF_SIZE)
    caught       = False    # determined at LED 3 trigger

    # Pre-roll: show initial 5s countdown before button is touched
    ...
    try:
        while True:
            if scan_btn.value() == 0:
                ...
                # ── At LED 3 (~1s in): roll the catch and start audio ──────
                if led_lit >= 3 and not audio_started:
                ...
                # ── Stream audio chunk each loop iteration ─────────────────
                if audio_started and audio and wav:
                    try:
                        num = wav.readinto(mono_buf)
                        if num > 0:
                            if num < MONO_BUF_SIZE:
                ...
                # ── Update OLED countdown only when second changes ─────────
                if secs_remaining != last_secs:
    ...
    # ── Throw animation — 3 white flashes ─────────────────────────────────────
    strip_clear(strip)
...
def identify_and_play(adc_val, oled, strip):
    ...
    elif 380 <= adc_val <= 420:   return play_umbreon(oled, strip)
...
def main():
    ...
    while True:
        ...
        # Wait for SCAN | long-press BOOT = reboot
        while True:
        ...
        if pokemon_key is not None:
            strip_clear(strip)
            sleep_ms(300)
            try_catch(oled, strip, scan_btn, pokemon_key, pokemon_name)
