...
FRAME_FILE      = "/sd/spheal.bin"
WAV_FILE        = "/sd/spheal.wav"
...
def draw_info(oled):
    oled.text("Spheal",     74,  0, 0)
    oled.text("----------", 70,  9, 0)
    oled.text("Water",      78, 18, 0)
    oled.text("Ice",        78, 27, 0)
    oled.text("----------", 70, 36, 0)
    oled.text("Hoenn",      78, 45, 0)
    oled.text("0.8m 42kg",  70, 55, 0)
...
def open_wav(path):
    ...
    if header[36:40] != b'data':
        # scan for data chunk if WAV has extra metadata
        while True:
            chunk = f.read(8)
...
try:
    ...
    if len(frames) == 0:
        raise Exception("No frames found in spheal.bin")
