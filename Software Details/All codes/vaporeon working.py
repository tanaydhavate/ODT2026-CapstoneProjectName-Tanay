...
AUDIO_RATE      = 8000
AUDIO_BUF_SIZE  = 16384
MONO_BUF_SIZE   = 1024      # doubled — fewer SD reads per second

FRAME_FILE      = "/sd/vaporeon.bin"
WAV_FILE        = "/sd/vaporeon.wav"
...
def init_sd():
    ...
    for attempt in range(5):
        try:
            ...
            print("SD attempt", attempt + 1, "failed:", e)
            sleep_ms(500)
    raise Exception("SD card failed after 5 attempts")
...
def show_vaporeon_colors(strip):
    for i in range(8):
        strip[i] = DARK_BLUE
...
try:
    ...
    if len(frames) == 0:
        raise Exception("No frames found in vaporeon.bin")
    ...
    # warm up I2S
    silence = bytearray(MONO_BUF_SIZE)
    for _ in range(16):
        audio.write(silence)
    del silence
    gc.collect()
    ...
    while True:
        ...
        if loop_count % 12 == 0:   # update OLED less often
